...
tmp.days <- subset (day.term, subset = "2021-01-01" <= date & date <= "2021-06-30")
plot (ave.temp.C ~ date, data = tmp.days, type = "l", col = "darkgrey",
      main = paste (Location, txt.ave.day.temp, latest.year),
      xlab = txt.time.days, ylab = txt.temp,
      ylim = c (min (tmp.days$min.temp.C, na.rm = TRUE),
...
tmp.days <- subset (day.term, subset = "2022-01-01" <= date & date <= "2022-06-30")
...
tmp.ref.days <- subset (cur.ref.days, subset = format (cur.ref.days$day, "%m") %in% c ("01", "02", "03", "04", "05", "06"))
...
latest.year <- 2022
tmp1 <- subset (day.term, subset = "2022-01-01" <= date & date <= "2022-06-30", select = c (date, ave.temp.C))
tmp1 <- cbind (d = format (tmp1$date, "%m-%d"), tmp1)
tmp2 <- subset (cur.ref.days,
                subset = "2022-01-01" <= paste0 (latest.year, format (day, "-%m-%d")) &
                                         paste0 (latest.year, format (day, "-%m-%d")) <= "2022-12-31")
...
legend ("topleft", 
        legend = c (txt.max, txt.ave, txt.min, latest.year),
...
legend ("topleft", 
        legend = c (txt.ave, "2022"), lty = c (1), col = c ("red", "black"))
