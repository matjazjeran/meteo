...
tmp.ref <- subset (ref.months.1981.2010, select = c (month.str, Mmean.ave.temp.C))
tmp.ref <- cbind (tmp.ref, key = substr (tmp.ref$month.str, start = 6, stop = 10))
tmp.curr <- subset (month.stat, subset = paste0 ("2022", "-01-01") <= month & month <= paste0 ("2022", "-11-30"))
tmp.curr <- cbind (tmp.curr, key = substr (tmp.curr$month.str, start = 6, stop = 10))
tmp <- merge (tmp.ref, tmp.curr, by = "key")
...
@
\end{figure}
Leto 2022 je imelo zelo topel februar (glede na referenčna leta od 1981 do 2010) in ekstremno vroče mesece maj, junij in julij. V juliju 2022 je bila dosežena so sedaj rekordna izračunana povprečna mesečna temperatura 24,45$^\circ$C v Ljubljani. V jeseni sta bila zelo topla meseca september in še bolj november, oktober pa je bil spet glede referenčnih oktobrov ekstremno topel.
\clearpage
\begin{figure}[ht]
<<eval=TRUE,echo=FALSE,results="asis",cache=FALSE,warning=FALSE,message=FALSE,comment=NA,out.width='100%'>>=
...
\end{center}
\end{figure}
Število dni med padavinami je 27.362 dneh med 01.01.1948 in 30.11.2022 bilo porazdeljeno takole:\\
%%\begin{table}[h]
%%\begin{center}
...
\hline
0 & 7379 & To število vključuje tudi več zaporednih dni s padavinami\\
1 & 1294\\
2 & 832\\
3 & 572\\
4 do 5 & 700\
...
V 333 dneh leta 2022 je med 01.01.2022 in 30.11.2022 so bile padavine in suše porazdeljene takole:\\
\begin{tabular}{c|r|l} 
Interval & Število dni v letu 2022 & Opombe\\
\hline
0 & 66 & To število vključuje tudi več zaporednih dni s padavinami\\
1 & 13\\
2 & 10\\
3 & 10\\
4 do 5 & 11\\
...
nato pa še 26.09.202 s 61 mm in 30.09.2022 s 69 mm.
...
V 109 dnevih s padavinami do 25 mm je padlo 49\% vseh padavin, kar 15\% vseh padavin v letu je padlo v enem dnevu v nalivu s 157 mm, v preostalih 8 dnevih s padavinami od 26 mm do 100 mm pa 27\% vseh padavin.
